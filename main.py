...
def build_index():
    ...
    try:
        ...
        dialogue_logs = db.collection('articles').where('sourceType', '==', 'DIALOGUE_LOG').stream()
        ...
        for log in dialogue_logs:
            ...
            jst_date_str = (created_at_dt.astimezone(timezone(timedelta(hours=9)))).strftime('%Y-%m-%d')
        ...
        return jsonify({"status": "success", "message": f"{len(logs_by_date)}日分のインデックスを更新しました。"}), 200
...
def process_daily_logs(date_str, logs):
    ...
    # 1. 1日のサマリーを生成 (AI)
    daily_summary = f"{date_str}の対話要約。この日は{len(logs)}件の対話記録がありました。"
